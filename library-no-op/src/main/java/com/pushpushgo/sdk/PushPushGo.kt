...
class PushPushGo private constructor(
    ...
    companion object {
        ...
        fun getInstance(application: Application) = INSTANCE ?: synchronized(this) {
            INSTANCE ?: PushPushGo(application, "", "", true, DEBUG, null).also { INSTANCE = it }
        }

        @JvmStatic
        @JvmOverloads
        fun getInstance(
            application: Application, apiKey: String, projectId: String, isProduction: Boolean, isDebug: Boolean = false, customBaseUrl: String?,
        ): PushPushGo {
            if (INSTANCE == null) {
                INSTANCE = PushPushGo(application, apiKey, projectId, isProduction, isDebug, customBaseUrl)
            }
